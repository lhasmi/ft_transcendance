...
const toTableState = () => {
	...
	if (upcomingMatches.value.length === 0 && players.value.length > 1) generateUpcomingMatches()
...
<template>
	<div class="container d-flex flex-grow-1 justify-content-center align-items-center ">
		...
		<div v-if="tournamentState === states.registration" class="myshadow bg-white bg-opacity-10 col-11 col-sm-10 col-md-8 col-lg-6 rounded-4">
			<p class="m-0 mt-3 mb-2 fs-3 roboto-bold text-center" style="color:#f58562">{{ getText('tournamentRegistration', store.lang) }}</p>
			...
			<ButtonComp @click="toTableState" class="btn-lg mt-3 mb-4 fs-5 d-flex justify-content-center mx-auto col-6 col-lg-4">{{ getText('ready', store.lang) }}</ButtonComp>
		</div>
		<!-- TOURNAMENT TABLE -->
		<div v-else-if="tournamentState === states.table" class="myshadow bg-white bg-opacity-10 col-11 col-sm-8 col-md-6 col-lg-4 rounded-4">
			<p class="m-0 mt-3 mb-2 fs-3 roboto-bold text-center" style="color:#f58562">{{ getText('upcomingMatches', store.lang) }}</p>
			<hr class="splitter col-12 mx-auto m-0 mb-4" />
			<div v-for="match in upcomingMatches" :key="match[0]" class="match-item col-11 col-sm-9 mx-auto">
				<div class="row">
					<div class="col text-end roboto-bold fs-4 text-white my-2 text-break align-self-center">{{ match[0] }}</div>
					<div class="col-2 text-center roboto-bold fs-4 my-2 align-self-center" style="color: #f58562;">vs</div>
					<div class="col text-start roboto-bold fs-4 text-white my-2 text-break align-self-center">{{ match[1] }}</div>
				</div>
				<!-- <p class="fs-4 m-0 my-2 text-white text-center roboto-bold">{{ match[0] }} <span style="color: #f58562;">vs</span> {{ match[1] }}</p> -->
			</div>
			<ButtonComp @click="toGameState" class="btn-lg mt-3 mb-4 fs-5 d-flex justify-content-center mx-auto col-7 col-lg-5">{{ getText('start', store.lang) }}</ButtonComp>
		</div>
		...
		<div v-else-if="tournamentState === states.end" class="myshadow bg-white bg-opacity-10 col-11 col-sm-8 col-md-6 col-lg-4 rounded-4">
			<p class="m-0 mt-3 mb-2 fs-3 roboto-bold text-center" style="color:#f58562">{{ getText('tournamentFinished', store.lang) }}</p>
			<hr class="splitter col-12 mx-auto m-0 mb-4" />
			<p class="m-0 mb-4 roboto-bold fs-4 text-white text-center text-break">
				<span class="roboto-bold" style="color: #f58562">{{ players[0] }}</span> {{ getText('isWinner', store.lang) }}
			</p>
		</div>
		...
		<div v-else class="myshadow bg-white bg-opacity-10 col-11 col-sm-9 col-md-7 col-lg-5 col-xl-4 rounded-4 d-flex flex-column align-items-center">
			<p class="fs-3 m-0 mt-3 mb-2 roboto-bold text-center" style="color: #f58562;">{{ getText('chooseAmountPlayers', store.lang) }}</p>
			<hr class="splitter col-12 mx-auto m-0 mb-4" />
			<div class="mb-4 d-flex">
				<ButtonComp @click="toRegistrationState(4)" class="btn-lg fs-3 me-4" style="width: 100px;">4</ButtonComp>
				<ButtonComp @click="toRegistrationState(8)" class="btn-lg fs-3 ms-4" style="width: 100px;">8</ButtonComp>
			</div>
		</div>
	</div>
