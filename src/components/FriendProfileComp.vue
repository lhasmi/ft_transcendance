...
const getCircleColor = (index) => {
  if (index >= props.data.gamesHistory.length) return 'background: rgba(255, 255, 255, 0.1)'
  return props.data.gamesHistory[index].score1 > props.data.gamesHistory[index].score2
    ? 'background: #66bf6a'
    : 'background: #da4834'
}
...
<template>
	<div class="modal-body p-0 d-flex flex-column" style="height: 396px">
		...
		<p class="col-9 col-md-7 mx-auto roboto-bold mb-2 mb-md-3 text-white fs-6">
			{{ getText('username', store.lang) }}: <span class="fs-6 ms-1">{{ props.data.username }}</span>
		</p>
		<div class="col-9 col-md-7 mx-auto roboto-bold mb-2 mb-md-3 text-white fs-6 d-flex ">
